...
def generate_from_labels(labels, model, sampler, config, device):
    n = labels.size(0)
    shape = (n, config.data.img_ch, config.data.img_size, config.data.img_size)
    samples = sampler.sample(model, shape, y=labels)
    return samples
